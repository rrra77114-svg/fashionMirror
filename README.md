...
基于树莓派5的强大性能，我们打造了这个充满人情味的 AI 伴侣。它不仅是镜子，更是懂得欣赏你的时尚知己。
...
| 特性     | 描述                         |
|--------|----------------------------|
| 实时人像检测 | 使用 OpenCV 进行面部检测，实时显示检测画面  |
| 智能穿衣夸奖 | 调用豆包 API 基于用户穿衣照片生成个性化夸奖文本 |
| 语音实时播报 | 通过火山引擎 TTS 将文本转换为语音并流式播放   |
...
## 演示
<https://www.bilibili.com/video/BV1ZpSMBVE6s>

## 使用
下载`fashionMirror.zip`<https://github.com/rrra77114-svg/fashionMirror/releases/tag/mirror>

安装所需库`setup.sh`或`setup_aliyun.sh`

在`fashionMirrorMain.py`中填入 API-KEY （OpenWeatherMap、豆包、火山TTS）可搜索`123`查找
...
导入必要的 Python 库，包括图像处理、网络请求、音频播放等模块  
...
- API 调用采用异步非阻塞方式，避免主线程被长时间阻塞。模块实现了完善的错误处理机制，包括网络超时、 API 限流、服务异常等多种情况的应对策略。当主要 API 服务不可用时，系统会自动切换到本地备用夸奖库，保证基本功能的可用性。备用夸奖库包含多个经过精心设计的夸奖模板，覆盖不同风格的穿衣搭配场景。
